                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="form-control w-full max-w-xs">
                        ...
                        <label class="input-group">
                            <input name="name" type="text" placeholder="{{ $recurrent->name }}" value="{{ $recurrent->name }}" class="input input-bordered input-primary w-full max-w-xs"/>
                        </label>
                    </div>
                    ...
                    <div class="form-control w-full max-w-xs">
                        ...
                        <label class="input-group">
                            <input name="amount" type="text" placeholder="{{ $recurrent->amount }}" value="{{ $recurrent->amount }}" class="input input-bordered" />
                            <span class="material-symbols-outlined">
                            euro
                        </span>
                        </label>
                        ...
                        <label class="input">
                            <select name="category_id" class="select select-bordered select-primary w-full max-w-xs">
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" @if($category->id == $recurrent->category_id) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </label>
                    </div>
            ...
            <div class="modal modal-bottom sm:modal-middle">
                <div class="modal-box">
                    <form method="POST" action="{{ 'recurrents/'. $recurrent->id }}">
                        ...
                        <p class="py-4">Are you sure you want to remove the recurrent? It will be deleted permanently!</p>
                        <div class="modal-action">
                            <label for="delete-recurrent-modal-{{$recurrent->id}}" class="btn btn-outline btn-error btn-sm">Cancel</label>
