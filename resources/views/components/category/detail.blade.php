@foreach($categories as $category)
    <!-- Open update modal -->
    <label for="update-category-modal-{{$category->id}}" class="cursor-pointer @if($category->is_expense == 0) text-success @else text-error @endif">
        {{ $category->name }}</label>
    ...
    <div class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
            ...
            <label for="update-category-modal-{{$category->id}}" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
            <form method="POST" action="{{ 'categories/'. $category->id }}">
                ...
                <div class="modal-action">
                    <!-- Open delete modal -->
                    <label for="delete-category-modal-{{$category->id}}" class="btn btn-outline btn-error btn-sm">Delete</label>
            ...
            <div class="modal modal-bottom sm:modal-middle">
                <div class="modal-box">
                    ...
                    <p class="py-4">Are you sure you want to delete the category? Any expenses or income related to this category will also be permanently removed!</p>
                    <div class="modal-action">
                        <label for="delete-category-modal-{{$category->id}}" class="btn btn-outline btn-error btn-sm">Cancel</label>
